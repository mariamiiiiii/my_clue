...
class LayerTilesGPU {
 public:
  ...
  __host__ __device__ int4 searchBox(float xMin, float xMax, float yMin,
                                     float yMax) {
  ...
  __host__ __device__ GPU::VecArray<int, LayerTilesConstants::maxTileDepth>&
  operator[](int globalBinId) {
    return layerTiles_[globalBinId];
  }

 private:
  GPU::VecArray<GPU::VecArray<int, LayerTilesConstants::maxTileDepth>,
