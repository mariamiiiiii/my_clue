    ...
    for i, (kind, key, pretty) in enumerate(groups):
        ...
        if kind == "simple":
            ...
            ax.annotate(fmt_mean_with_error(u_exe, u_exe_std), (xu, y_at_std_top(u_exe, u_exe_std)), xytext=(0, TOP_DY + 10),
                        textcoords="offset points", ha="center", va="bottom", fontsize=FS_TOP)
            ax.annotate(f"{u_sub:.3f}", (xu, u_sub), xytext=(0, -25), textcoords="offset points", ha="center", va="center", fontsize=FS_TOP, color="white", bbox=dict(boxstyle="round,pad=0.3", fc="black", ec="none", alpha=0.7))
            ax.annotate(fmt_mean_with_error(n_exe, n_exe_std), (xn, y_at_std_top(n_exe, n_exe_std)), xytext=(0, TOP_DY),
                        textcoords="offset points", ha="center", va="bottom", fontsize=FS_TOP)
            ax.annotate(f"{n_sub:.3f}", (xn, n_sub/2), xytext=(0, -10), textcoords="offset points", ha="center", va="center", fontsize=FS_TOP, color="white", bbox=dict(boxstyle="round,pad=0.3", fc="black", ec="none", alpha=0.7))
