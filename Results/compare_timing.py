...
# Save the order from the first file (normalized)
unified_order_raw = unified_dfs[0]['Operation'].tolist()
unified_order = [normalize_operation(x) for x in unified_order_raw]
...
# Save the order from the first file (normalized)
unified_no_prefetch_order_raw = unified_no_prefetch_dfs[0]['Operation'].tolist()
unified_no_prefetch_order = [normalize_operation(x) for x in unified_no_prefetch_order_raw]

# Normalize op names BEFORE concat
for df in unified_no_prefetch_dfs:
    df['Operation'] = df['Operation'].map(normalize_operation)
    ...
    for i, (kind, key, pretty) in enumerate(groups):
        ...
        if kind == "simple":
            ...
            ax.annotate(fmt_mean_with_error(n_exe, n_exe_std), (xn, y_at_std_top(n_exe, n_exe_std)), xytext=(0, TOP_DY),
                        textcoords="offset points", ha="center", va="bottom", fontsize=FS_TOP)
            ax.annotate(f"{n_sub:.1f}", (xn, n_sub), xytext=(0, -10), textcoords="offset points", ha="center", va="center", fontsize=FS_TOP, color="white", bbox=dict(boxstyle="round,pad=0.3", fc="black", ec="none", alpha=0.7))
