...
class CLUEAlgoGPU : public CLUEAlgo<T, NLAYERS> {
 ...
 public:
  ...
  void init_input_data(bool memadvise) {
    ...
    CHECK_HIP_ERROR(hipMallocManaged(&d_points.x, sizeof(float) * reserve));
    if (memadvise)
      CHECK_HIP_ERROR(hipMemAdvise(d_points.x, sizeof(float) * reserve, hipMemAdviseSetReadMostly, device_));

    CHECK_HIP_ERROR(hipMallocManaged(&d_points.y, sizeof(float) * reserve));
    if (memadvise)
      CHECK_HIP_ERROR(hipMemAdvise(d_points.y, sizeof(float) * reserve, hipMemAdviseSetReadMostly, device_));

    CHECK_HIP_ERROR(hipMallocManaged(&d_points.layer, sizeof(int) * reserve));
    if (memadvise)
      CHECK_HIP_ERROR(hipMemAdvise(d_points.layer, sizeof(int) * reserve, hipMemAdviseSetReadMostly, device_));

    CHECK_HIP_ERROR(hipMallocManaged(&d_points.weight, sizeof(float) * reserve));
    if (memadvise)
      CHECK_HIP_ERROR(hipMemAdvise(d_points.weight, sizeof(float) * reserve, hipMemAdviseSetReadMostly, device_));

    if (useAbsoluteSigma_) {
      CHECK_HIP_ERROR(hipMallocManaged(&d_points.sigmaNoise, sizeof(float) * reserve));
      if (memadvise)
        CHECK_HIP_ERROR(hipMemAdvise(d_points.sigmaNoise, sizeof(float) * reserve, hipMemAdviseSetReadMostly, device_));
    }
  }

  void init_output_data() { 
    unsigned int reserve = 1000000;
    CHECK_HIP_ERROR(hipMallocManaged(&d_points.rho, sizeof(float) * reserve));
    CHECK_HIP_ERROR(hipMallocManaged(&d_points.delta, sizeof(float) * reserve));
    CHECK_HIP_ERROR(hipMallocManaged(&d_points.nearestHigher,sizeof(int) * reserve));
    CHECK_HIP_ERROR(hipMallocManaged(&d_points.clusterIndex,sizeof(int) * reserve));
    CHECK_HIP_ERROR(hipMallocManaged(&d_points.isSeed, sizeof(uint8_t) * reserve));
  }
  ...
  void copy_todevice(bool prefetch) {
    ...
    if (prefetch)
      CHECK_HIP_ERROR(hipMemPrefetchAsync(d_points.x, sizeof(float) * points_.n, device_, stream_));

    memcpy(d_points.y, points_.p_y, sizeof(float) * points_.n);
    if (prefetch)
      CHECK_HIP_ERROR(hipMemPrefetchAsync(d_points.y, sizeof(float) * points_.n, device_, stream_));

    memcpy(d_points.layer, points_.p_layer, sizeof(int) * points_.n);
    if (prefetch)
      CHECK_HIP_ERROR(hipMemPrefetchAsync(d_points.layer, sizeof(int) * points_.n, device_, stream_));

    memcpy(d_points.weight, points_.p_weight, sizeof(float) * points_.n);
    if (prefetch)
      CHECK_HIP_ERROR(hipMemPrefetchAsync(d_points.weight, sizeof(float) * points_.n, device_, stream_));

    if (useAbsoluteSigma_) {
      memcpy(d_points.sigmaNoise, points_.p_sigmaNoise, sizeof(float) * points_.n);
      if (prefetch)
        CHECK_HIP_ERROR(hipMemPrefetchAsync(d_points.sigmaNoise, sizeof(float) * points_.n, device_, stream_));
    }
  }
  ...
  void copy_tohost(bool prefetch) {
    // result variables
    if (prefetch)
      CHECK_HIP_ERROR(hipMemPrefetchAsync(d_points.clusterIndex, sizeof(int) * points_.n, hipCpuDeviceId, stream_));
    memcpy(points_.clusterIndex.data(), d_points.clusterIndex, sizeof(int) * points_.n);

    if (verbose_) {
      // other variables, copy only when verbose_==True
      if (prefetch)
        CHECK_HIP_ERROR(hipMemPrefetchAsync(d_points.rho, sizeof(float) * points_.n, hipCpuDeviceId, stream_));
      memcpy(points_.rho.data(), d_points.rho, sizeof(float) * points_.n);

      if (prefetch)
        CHECK_HIP_ERROR(hipMemPrefetchAsync(d_points.delta, sizeof(float) * points_.n, hipCpuDeviceId, stream_));
      memcpy(points_.delta.data(), d_points.delta, sizeof(float) * points_.n);

      if (prefetch)
        CHECK_HIP_ERROR(hipMemPrefetchAsync(d_points.nearestHigher, sizeof(int) * points_.n, hipCpuDeviceId, stream_));
      memcpy(points_.nearestHigher.data(), d_points.nearestHigher, sizeof(int) * points_.n);

      if (prefetch)
        CHECK_HIP_ERROR(hipMemPrefetchAsync(d_points.isSeed, sizeof(uint8_t) * points_.n, hipCpuDeviceId, stream_));
  ...
  void free_input_data() {
    CHECK_HIP_ERROR(hipFree(d_points.x));
    CHECK_HIP_ERROR(hipFree(d_points.y));
    CHECK_HIP_ERROR(hipFree(d_points.layer));
    CHECK_HIP_ERROR(hipFree(d_points.weight));
    if (useAbsoluteSigma_)
      CHECK_HIP_ERROR(hipFree(d_points.sigmaNoise));
  }

  void free_output_data() {
    CHECK_HIP_ERROR(hipFree(d_points.rho));
    CHECK_HIP_ERROR(hipFree(d_points.delta));
    CHECK_HIP_ERROR(hipFree(d_points.nearestHigher));
    CHECK_HIP_ERROR(hipFree(d_points.clusterIndex));
    CHECK_HIP_ERROR(hipFree(d_points.isSeed));
  }
 ...
 private:
  ...
  void init_internal_data() {
    CHECK_HIP_ERROR(hipGetDevice(&device_));
