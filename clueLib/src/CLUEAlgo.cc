...
void CLUEAlgo::verboseResults(const std::string& outputFileName /* "cout" */,
  ...
  for (unsigned i = 0; i < to_print; i++) {
    snprintf(buffer, 100,
             ...
             points_.x[i], points_.y[i], points_.layer[i], points_.weight[i],
...
void CLUEAlgo::prepareDataStructures(
    std::array<LayerTiles, NLAYERS>& allLayerTiles) {
...
void CLUEAlgo::calculateLocalDensity(
    std::array<LayerTiles, NLAYERS>& allLayerTiles) {
  ...
  for (unsigned i = 0; i < points_.n; i++) {
    ...
    std::array<int, 4> search_box =
        lt.searchBox(points_.x[i] - dc_, points_.x[i] + dc_, points_.y[i] - dc_,
                     points_.y[i] + dc_);
...
void CLUEAlgo::calculateDistanceToHigher(
    std::array<LayerTiles, NLAYERS>& allLayerTiles) {
  // loop over all points
